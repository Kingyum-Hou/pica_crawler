...
### Ver 1.1
* 文件名过长问题
* 过滤器和已下载章节判定器分离：修改filter_comics函数
* 记录更多的comic info
* 人工查询一些ERROR中无法获取pageCount是否为网络问题，还是其它原因（找不到原因，直接干算了）
* 每个系列下生成一个记录基本漫画信息的json文件，发现了一个严重bug，会导致同名作者下多个章节不能同时保存，已修正
* 更新原data数据库
* 下载后转移comic文件到备份目录
